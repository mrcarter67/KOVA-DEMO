...
function fv(n:number){ const abs=Math.abs(n).toLocaleString("en-US",{minimumFractionDigits:0,maximumFractionDigits:0}); return (n<0?"-":"")+"$"+abs; }
function pv(n:number){ return n.toLocaleString("en-US",{minimumFractionDigits:0,maximumFractionDigits:0}); }
...
export default function FinancialHub({ vertId }:{ vertId:string }) {
  ...
  const sendMsg = async(text:string) => {
    ...
    try {
      ...
      const fallbacks: Record<string,string> = {
        "margin":"Your gross margin dropped 3.5 points over 90 days — from 34% to 30.5%. The driver is labor cost inflation, not revenue softness. Your revenue is actually up. The fix is in your estimating templates — bump labor cost assumptions by 12% before you quote any new jobs. I'd also look at the two jobs that ran overtime in May: if those had been estimated correctly, your margin holds at 32%.",
        "overdue":"Three invoices: INV-2026-202 ($1,610 — 36 days), INV-2026-198 ($22,400 — 28 days), INV-2026-189 ($24,590 — 22 days). Total: $48,600. The big risk is INV-2026-198. At $22K and 28 days out, that's the one to call today — not email. The $24.5K one is still within window but should get a written follow-up by Friday.",
        "material":"Materials jumped from $15,800 in April to $18,600 in May — an 18% increase driven primarily by lumber and copper pricing. This is a market condition, not a vendor issue. Short-term: add a material cost escalation clause to any job over $10K. Longer-term: lock in pricing on a 90-day supply agreement with your two primary vendors before Q3 jobs start.",
        "cash":"You have $186,400 on hand. At current monthly burn of $71,200, that's about 2.6 months of runway without any new revenue. But you're bringing in $102K monthly, so this is healthy. The risk scenario: if those 3 overdue invoices go 60 days, and you add two more slow payers, your cushion shrinks fast. Get the AR collected and you're in strong shape for summer.",
      };
      const key = Object.keys(fallbacks).find(k=>text.toLowerCase().includes(k));
      setMsgs(m=>[...m,{role:"assistant",content:key?fallbacks[key]:"Your financials are strong overall — $102K in June, 30.5% margin, $186K cash. The three areas that need attention: collect the $48.6K in overdue AR, adjust your estimating for the new material cost baseline, and review the jobs that ran overtime in May. Want me to walk through any of these in detail?"}]);
    }
    setLoading(false);
  };
  ...
  return (
    <div>
      {/* KPI header */}
      <div style={{background:"linear-gradient(135deg,#0F172A,#1E3A5F)",borderRadius:12,padding:"14px 16px",marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            <div style={{fontSize:11,color:"rgba(255,255,255,0.5)",textTransform:"uppercase",letterSpacing:"2px",marginBottom:2}}>Financial Hub · Atlas AI</div>
            <div style={{fontSize:18,fontWeight:700,color:"#F1F5F9"}}>June 2026 — Performance Overview</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:24,fontWeight:700,color:"#00C896"}}>$102,400</div>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.4)"}}>Monthly Revenue · +8.7% MoM</div>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7}}>
          {KPIs.slice(0,6).map(k=>(
            <div key={k.label} style={{background:"rgba(255,255,255,0.06)",borderRadius:8,padding:"9px 12px"}}>
              ...
              <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",marginTop:1}}>{k.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Tab bar */}
      <div style={{display:"flex",gap:0,borderBottom:"1px solid #E2E8F0",background:"#fff",borderRadius:"8px 8px 0 0",marginBottom:0}}>
        {TABS.map(([t,l])=>(
          <button key={t} onClick={()=>setTab(t as any)} style={{padding:"9px 14px",fontSize:12,fontWeight:tab===t?600:400,border:"none",background:"none",cursor:"pointer",color:tab===t?"#1D4ED8":"#64748B",borderBottom:tab===t?"2px solid #1D4ED8":"2px solid transparent",whiteSpace:"nowrap"}}>{l}</button>
        ))}
      </div>

      <div style={{background:"#fff",border:"1px solid #E2E8F0",borderTop:"none",borderRadius:"0 0 8px 8px",padding:"14px 16px",minHeight:400}}>
        ...
        {tab==="overview" && (
          <div>
            ...
            <div style={{marginBottom:16}}>
              <div style={{fontSize:11,fontWeight:700,color:"#0F172A",marginBottom:10}}>Revenue vs Expenses — 6 Month Trend</div>
                ...
                {MONTHS.map((m,i)=>(
                  <div key={m} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                    <div style={{width:"100%",display:"flex",gap:2,alignItems:"flex-end",height:100}}>
                      ...
                      <div style={{flex:1,background:"#EF4444",opacity:0.7,borderRadius:"3px 3px 0 0",height:`${EXPENSES[i]/BAR_MAX*100}%`,minHeight:4}} title={`Expenses: ${fv(EXPENSES[i])}`} />
                    </div>
              ...
              <div style={{display:"flex",gap:12,fontSize:10,color:"#64748B"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:4}}><div style={{width:10,height:10,background:"#EF4444",opacity:0.7,borderRadius:2}} />Expenses</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div>
                <div style={{fontSize:10,fontWeight:700,color:"#15803D",textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:7}}>📈 Uptakes — What's Growing</div>
                {UPTAKES.map((u,i)=>(
                  <div key={i} style={{padding:"10px 11px",background:"#F0FDF4",border:"1px solid #86EFAC",borderRadius:8,marginBottom:6}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                      <div style={{display:"flex",gap:5,alignItems:"center"}}>
                        <span style={{fontSize:12}}>{u.icon}</span>
                        <span style={{fontSize:11,fontWeight:700,color:"#15803D"}}>{u.title}</span>
                      </div>
                      <span style={{fontSize:10,fontWeight:700,color:"#15803D",background:"#DCFCE7",padding:"1px 6px",borderRadius:99}}>{u.value}</span>
                    </div>
                    <div style={{fontSize:10,color:"#166534",lineHeight:1.5}}>{u.body}</div>
                  </div>
                ))}
              </div>
              <div>
                <div style={{fontSize:10,fontWeight:700,color:"#B91C1C",textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:7}}>📉 Downtakes — Watch These</div>
                {DOWNTAKES.map((d,i)=>(
                  <div key={i} style={{padding:"10px 11px",background:"#FEF2F2",border:"1px solid #FCA5A5",borderRadius:8,marginBottom:6}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                      <div style={{display:"flex",gap:5,alignItems:"center"}}>
                        <span style={{fontSize:12}}>{d.icon}</span>
                        <span style={{fontSize:11,fontWeight:700,color:"#B91C1C"}}>{d.title}</span>
                      </div>
                      <span style={{fontSize:10,fontWeight:700,color:"#B91C1C",background:"#FEE2E2",padding:"1px 6px",borderRadius:99}}>{d.value}</span>
                    </div>
                    <div style={{fontSize:10,color:"#991B1B",lineHeight:1.5}}>{d.body}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {tab==="pl" && (
          <div>
            <div style={{fontSize:11,fontWeight:700,color:"#0F172A",marginBottom:10}}>Profit & Loss — June 2026</div>
            {/* Revenue breakdown */}
            <div style={{marginBottom:14}}>
              ...
              {REVENUE_CATS.map(r=>(
                <div key={r.label} style={{marginBottom:5}}>
                  ...
                  <div style={{height:4,background:"#F1F5F9",borderRadius:99}}><div style={{height:"100%",background:r.color,borderRadius:99,width:`${r.pct}%`}} /></div>
                </div>
              ))}
            </div>
            ...
            <div style={{marginBottom:14}}>
              ...
              {EXPENSE_CATS.map(e=>(
                <div key={e.label} style={{marginBottom:5}}>
                  ...
                  <div style={{height:4,background:"#F1F5F9",borderRadius:99}}><div style={{height:"100%",background:e.color,borderRadius:99,width:`${e.pct}%`}} /></div>
                </div>
              ))}
            </div>
            ...
            {[
              ...
              <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:l==="Gross Margin"?"none":"1px solid #F8FAFC"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
              {[["Cash In","$72,180","#10B981","↑"],["Cash Out","($54,260)","#EF4444","↓"],["Net Cash","$17,920","#3B9EFF","→"]].map(([l,v,c,a])=>(
                <div key={l as string} style={{padding:"12px",background:"#F8FAFC",borderRadius:8,textAlign:"center"}}>
                  <div style={{fontSize:10,color:"#64748B",marginBottom:3}}>{a} {l}</div>
                  <div style={{fontSize:18,fontWeight:700,color:c as string}}>{v}</div>
                  <div style={{fontSize:9,color:"#94A3B8"}}>This month</div>
                </div>
              ))}
            </div>
            <div style={{fontSize:11,fontWeight:700,color:"#0F172A",marginBottom:8}}>Recent Transactions</div>
            {RECENT_TRANSACTIONS.map((t,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:"1px solid #F8FAFC"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:10,color:"#94A3B8"}}>{t.date} · {t.cat}</div>
                </div>
                <div style={{fontSize:13,fontWeight:700,color:t.type==="in"?"#15803D":"#B91C1C"}}>{fv(t.amount)}</div>
              </div>
            ))}
            {/* AR aging */}
            <div style={{marginTop:14,padding:"12px",background:"#FEF2F2",border:"1px solid #FCA5A5",borderRadius:8}}>
              <div style={{fontSize:10,fontWeight:700,color:"#B91C1C",marginBottom:6}}>⚠ Accounts Receivable Aging</div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",paddingTop:6}}>
                ...
                <span style={{fontSize:12,fontWeight:700,color:"#B91C1C"}}>$48,600</span>
              </div>
            </div>
          </div>
        )}
        ...
        {tab==="atlas" && (
          <div style={{display:"flex",flexDirection:"column",height:480}}>
            {/* Agent header */}
            <div style={{padding:"10px 0",borderBottom:"1px solid #F1F5F9",marginBottom:10,display:"flex",gap:8,alignItems:"center"}}>
              <div style={{width:32,height:32,borderRadius:"50%",background:"#6366F115",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>📊</div>
              <div>
            ...
            <div style={{display:"flex",gap:4,marginBottom:10,flexWrap:"wrap"}}>
              {["Why is my margin dropping?","What are my overdue invoices?","How are my material costs?","What's my cash position?"].map(q=>(
                <button key={q} onClick={()=>sendMsg(q)} style={{fontSize:9,padding:"3px 9px",borderRadius:99,border:"1px solid #E2E8F0",background:"#F8FAFC",cursor:"pointer",color:"#64748B"}}>{q}</button>
              ))}
            </div>
            ...
            <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:9,marginBottom:10}}>
              {msgs.map((m,i)=>(
                <div key={i} style={{display:"flex",gap:7,justifyContent:m.role==="user"?"flex-end":"flex-start",alignItems:"flex-end"}}>
                  {m.role==="assistant"&&<div style={{width:26,height:26,borderRadius:"50%",background:"#6366F115",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,flexShrink:0}}>📊</div>}
              ...
              {loading&&(
                <div style={{display:"flex",gap:7,alignItems:"flex-end"}}>
                  <div style={{width:26,height:26,borderRadius:"50%",background:"#6366F115",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12}}>📊</div>
                  <div style={{padding:"9px 13px",background:"#F8FAFC",borderRadius:"9px 9px 9px 2px",display:"flex",gap:3}}>
                    {[0,1,2].map(j=><div key={j} style={{width:5,height:5,borderRadius:"50%",background:"#6366F1",animation:`pulse 1.2s ${j*0.2}s ease-in-out infinite`}} />)}
                  </div>
                </div>
              )}
              <div ref={bottomRef} />
            ...
            <div style={{display:"flex",gap:7}}>
              <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendMsg(input)} disabled={loading}
                placeholder="Ask Atlas about your financials…"
                style={{flex:1,padding:"9px 12px",border:"1px solid #E2E8F0",borderRadius:8,fontSize:12,background:"#F8FAFC"}} />
              <button onClick={()=>sendMsg(input)} disabled={loading||!input.trim()} style={{padding:"9px 18px",background:input.trim()&&!loading?"#6366F1":"#E2E8F0",color:input.trim()&&!loading?"#fff":"#94A3B8",border:"none",borderRadius:8,fontSize:11,fontWeight:700,cursor:"pointer"}}>Send</button>
            </div>
          </div>
        )}
      </div>
